...
if [[ $WRT_REPO == *"lede"* ]]; then
	...
	#添加编译日期标识
	CURRENT_TIME=$(date +"%Y-%m-%d %H:%M:%S") && sed -i "s/(L.isObject(boardinfo.release)?boardinfo.release.description\+' \/ '\:'')\+(luciversion\|\|'')/(' $WRT_NAME ') + (' \/ 布丁智能科技 © 蓝色的海 compiled in $CURRENT_TIME')/g" $(find ./feeds/luci/modules/luci-mod-status/ -type f -name "10_system.js")
fi
